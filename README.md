...
**Randomized Gym Challenge** is a configurable gym-battle mode for Pokémon Red, Blue, Yellow, Gold, and Silver in Gen 1 Recomp. It is not a replacement for the normal game and it is not a ROM hack. It builds a persistent challenge plan for the gyms in a save, while each physical gym keeps its normal badge, reward, and progression event.
...
| Randomize Held Items (Gen 2) | Gives generated Gold or Silver gym Pokémon a safe held item or no item. Key items, HMs, non-tossable items, and no-effect held items are excluded. |
...
**Gym Challenge** is optional. In Red, Blue, and Yellow, a Yes/No text box appears immediately after the player-name screen in the normal opening. Choosing **Yes** only records the opt-in; the starter, Oak’s Lab rival, parcel delivery, Pokédex award, and rival departure remain fully native. At the final command of that completed parcel/Pokédex cutscene, the mod automatically begins the accepted challenge. Choosing **No** leaves normal game flow unchanged. In Gold and Silver, the established opt-in still appears only after the player has received a starter, returned to Elm’s Lab with the Mystery Egg, and handed that egg to Elm.

In Gen 1, the completed parcel/Pokédex cutscene first shows a concise explanation of Gym Challenge, then a start summary listing the first gym, saved difficulty preset, and enabled rules. The mod safely updates the already received native starter, restores party HP without refilling move PP, and routes the player to the first physical gym. It does not replace either game’s native starter or early-story scripts.
...
Please test a new save for the Gen 1 Gym Challenge decision flow. The Gen 1 choice is intentionally made only after player naming; existing saves that have passed the opening do not receive a retroactive Oak’s Lab prompt. Confirm that the opening leader text, overworld sprite, generated party, physical-gym reward, save/reload behavior, and challenge log agree in at least an early, mid-game, and late-game gym. On Gold or Silver, also test a Johto gym and a Kanto gym, then repeat with held-item randomization enabled.

For Gym Challenge, verify that no prompt appears before or during Oak’s introduction. In Gen 1, verify that the Yes/No choice appears immediately after naming the player; choose both answers on separate new saves. With Yes selected, verify that all opening events remain native, then that the completed Oak’s Parcel/Pokédex cutscene shows the challenge explanation and start summary before the HP-only first-gym warp. Also test the three type-adjustment outcomes, each post-reward continuation answer, the deferred Gym Guide continuation after a Pokémon Center visit, preservation of move PP, the eighth-badge Route 23 handoff before all badge guards, each Gym Guide’s first-talk reward and repeat talk, bounded reward quantities, a full item bag, save/reload before and after a guide reward, Progress History route status, paused-route diagnostics, the confirmation-gated abandon action, all four presets before plan creation, Gen 1 gym-trainer dialogue and statue names, Gen 2 guide-less fallback rewards, Gold or Silver’s first eight gyms followed by the native first Elite Four, Champion, Hall of Fame, and credits, the Continue prompt on the next New Bark boot, both Continue answers, Kanto gyms 9–16, the second native Elite Four and Champion, and final challenge completion. If Crystal 251 is active, repeat at least one early and one late gym with imported species and trainer data.
